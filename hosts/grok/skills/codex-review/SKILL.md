---
name: codex-review
description: Use when the user wants Codex, through AgentBridge, to review current changes, a branch diff, or adversarially challenge a design for Grok Build.
---
...
- User asked for fixes/edits (use `codex-rescue` with explicit write intent instead).
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" codex review --json
```

Adversarial / focused review:

```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" codex adversarial-review --json --prompt "$FOCUS"
```
