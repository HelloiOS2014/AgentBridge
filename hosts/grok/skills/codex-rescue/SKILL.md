---
name: codex-rescue
description: Use when the user wants Codex, through AgentBridge, to investigate a failure or explicitly implement/fix something for Grok Build.
---

# Codex Rescue (AgentBridge)

Delegate investigation or **explicitly write-enabled** work to **Codex**.

## When To Use

- Investigate failures, propose fixes (default **read-only**).
- Implement/fix **only** when the user clearly asks Codex to change code (then add `--write`).

## Safety

- Default: **no** `--write` (diagnosis / dry-run).
- Add `--write` **only** when the user explicitly wants Codex to edit/fix/implement.
...
AB_PLUGIN_VERSION="$(find "$HOME/.grok" -path "*codex-bridge*" -name version -type f -not -path "*/.git/*" 2>/dev/null | sort -V | tail -n1)"
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" codex rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" codex rescue --write --json --prompt "$PROMPT"
```
