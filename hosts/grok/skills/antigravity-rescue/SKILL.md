...
description: Use when the user wants Antigravity, through AgentBridge, to investigate a failure or explicitly implement/fix something for Grok Build.
---
...
AB_PLUGIN_VERSION="$(find "$HOME/.grok" -path "*antigravity-bridge*" -name version -type f -not -path "*/.git/*" 2>/dev/null | sort -V | tail -n1)"
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" antigravity rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" antigravity rescue --write --json --prompt "$PROMPT"
```
