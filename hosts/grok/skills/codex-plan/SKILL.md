---
name: codex-plan
description: Use when the user wants Codex, through AgentBridge, to produce architecture plans, specs, sequencing, risk analysis, or implementation strategy for Grok Build.
---
...
- User asks Grok Build to have Codex plan architecture, design, rollout, risks, or sequencing.
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-grok" codex plan --json --prompt "$PROMPT"
```
...
- Do **not** implement the plan unless the user explicitly asks Grok Build to implement (or explicitly asks write-enabled rescue on Codex).
