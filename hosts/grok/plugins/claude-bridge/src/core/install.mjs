...
/**
 * CJS wrapper body (works without a package.json "type").
 * cliPath 可传 JS 表达式字符串（options.rawExpression）——插件内静态 wrapper 用
 * `require("node:path").join(require("node:os").homedir(), ...)` 运行时解析 $HOME。
 * @param {string} host
 * @param {string} cliPath
 * @param {{ rawExpression?: boolean }} [options]
 */
export function wrapperBody(host, cliPath, options = {}) {
  const cli = options.rawExpression ? cliPath : JSON.stringify(cliPath);
...
export function writeHostWrapper(host, env = process.env) {
  ...
  const body = wrapperBody(host, cliPath);
