...
description: Use when checking AgentBridge setup, job status, results, or cancellation for Claude Code work started from Codex.
---
...
The wrapper and engine self-bootstrap from the claude-bridge plugin on first use (no manual install command).

# Self-bootstrap: install engine from plugin on first use (idempotent, version-guarded)
AB_PLUGIN_VERSION="$(find "$HOME/.codex" -path "*claude-bridge*" -name version -type f -not -path "*/.git/*" 2>/dev/null | sort -V | tail -n1)"
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" claude setup --json
"$HOME/.agent-bridge/bin/agent-bridge-codex" doctor --host codex --json
```
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" status "$JOB_ID" --json
"$HOME/.agent-bridge/bin/agent-bridge-codex" result "$JOB_ID" --json
"$HOME/.agent-bridge/bin/agent-bridge-codex" cancel "$JOB_ID" --json
```
