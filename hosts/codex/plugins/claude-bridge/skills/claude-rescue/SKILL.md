---
name: claude-rescue
description: Use when the user wants Claude Code, through AgentBridge, to investigate a failure or explicitly implement/fix something for Codex.
---
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" claude rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" claude rescue --write --json --prompt "$PROMPT"
```
