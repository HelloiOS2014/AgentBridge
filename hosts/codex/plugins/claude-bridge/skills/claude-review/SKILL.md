---
name: claude-review
description: Use when the user wants Claude Code, through AgentBridge, to review current changes, a branch diff, or adversarially challenge a design for Codex.
---
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" claude review --json
```

Adversarial / focused review:

```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" claude adversarial-review --json --prompt "$FOCUS"
```
