...
description: Use when the user wants Antigravity, through AgentBridge, to investigate a failure or explicitly implement/fix something for Codex.
---
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" antigravity rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" antigravity rescue --write --json --prompt "$PROMPT"
```
