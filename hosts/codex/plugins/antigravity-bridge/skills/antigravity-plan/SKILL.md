...
description: Use when the user wants Antigravity, through AgentBridge, to produce architecture plans, specs, sequencing, risk analysis, or implementation strategy for Codex.
---
...
- User asks Codex to have Antigravity plan architecture, design, rollout, risks, or sequencing.
...
Use the host wrapper (installed by the antigravity-bridge plugin's self-bootstrap on first use; no manual install command needed). Prefer the absolute path below; do not require the user to export env vars.

# Self-bootstrap: install engine from plugin on first use (idempotent, version-guarded)
AB_PLUGIN_VERSION="$(find "$HOME/.codex" -path "*antigravity-bridge*" -name version -type f -not -path "*/.git/*" 2>/dev/null | sort -V | tail -n1)"
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-codex" antigravity plan --json --prompt "$PROMPT"
```
...
- Do **not** implement the plan unless the user explicitly asks Codex to implement (or explicitly asks write-enabled rescue on Antigravity).
