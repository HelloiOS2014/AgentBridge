---
name: codex-rescue
description: Use when the user wants Codex, through AgentBridge, to investigate a failure or explicitly implement/fix something for Claude Code.
---
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-claude" codex rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-claude" codex rescue --write --json --prompt "$PROMPT"
```
