...
description: Use when the user wants Antigravity, through AgentBridge, to investigate a failure or explicitly implement/fix something for Claude Code.
---

# Antigravity Rescue (AgentBridge)

Delegate investigation or **explicitly write-enabled** work to **Antigravity**.

## When To Use

- Investigate failures, propose fixes (default **read-only**).
- Implement/fix **only** when the user clearly asks Antigravity to change code (then add `--write`).

## Safety

- Default: **no** `--write` (diagnosis / dry-run).
- Add `--write` **only** when the user explicitly wants Antigravity to edit/fix/implement.
...
AB_PLUGIN_VERSION="$(find "$HOME/.claude/plugins" -path "*antigravity-bridge*" -name version -type f -not -path "*/.git/*" 2>/dev/null | sort -V | tail -n1)"
...
```bash
"$HOME/.agent-bridge/bin/agent-bridge-claude" antigravity rescue --json --prompt "$PROMPT"
```

Write-enabled (explicit user intent only):

```bash
"$HOME/.agent-bridge/bin/agent-bridge-claude" antigravity rescue --write --json --prompt "$PROMPT"
```
